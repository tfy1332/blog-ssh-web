...
public void setSessionFactory(SessionFactory sessionFactory) {
	...
	public void saveUserpwByEmail(String email, String password) throws Exception {
		 ...
		 */
		Session session=sessionFactory.openSession();
		String hql="update User u  set u.password=:newPassword where u.email=:email"; 
		Query query=session.createQuery(hql);     
		 query.setParameter("newPassword",password,Hibernate.STRING);   
		 query.setParameter("email",email,Hibernate.STRING);   
		 query.executeUpdate();
		 session.close();
	}
	...
	public User getUserByEmail(String email) throws Exception {
		List<User> userList= hibernateTemplate.find("from User u where u.email= '" + email+"'");
		User user=null;
		if(userList!=null &&userList.size()>0){
			 user=userList.get(0);
		}
