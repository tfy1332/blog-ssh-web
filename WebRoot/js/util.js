...
$.extend($.fn.form.methods, {
	/**
	 * 为form添加getData方法 获取格式化的表单数据
	 * @param jq
	 * @param params
	 * @returns {___anonymous2160_2161}
	 */
    getData: function(jq, params){
    	var formArray = jq.eq(0).mySerializeArray();
        var oRet = {};
        for (var i in formArray) {
        	var vaule = $.trim(formArray[i].value);
        	if(formArray[i].trim == 'false') vaule = formArray[i].value;
        	if (typeof(oRet[formArray[i].name]) == 'undefined') {
                if (params) {
                    oRet[formArray[i].name] = (formArray[i].value == "true" || formArray[i].value == "false") ? formArray[i].value == "true" : vaule;
                }
                else {
                    oRet[formArray[i].name] = vaule;
                }
            }
            else {
                if (params) {
