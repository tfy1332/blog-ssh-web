						...
						"Delete" : function() {
							...
							$.ajax({
					    		   ...
					    		   success: function(msg){
					    					...
					    					alert("删除成功!");
					    					window.location.href="article!showAllArticleForPageAdmin.action";
					    		   }
					    	});
      ...
      <div class="row-fluid">
        ...
        <button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"
			...
			<table class="table  table-hover">
				...
				<c:forEach items="${pager.list}" varStatus="articles" var="article">
					<tr class="success">
						...
						<td><a href="user!showAllUserForPage?username=${article.authorName} ">${article.authorName} </a></td>
			...
			<div class="pagination" align="center">
				...
				<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=1">首页</a></li>
				<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex-1}">Prev</a></li>
				</c:if>
				<c:if test="${pager.pageIndex>=6}">
				<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex-1}">...</a></li>
				</c:if>
				<c:if test="${pager.pageIndex<=pager.totalPage }">
				<c:choose>
      			 <c:when test="${pager.pageIndex%5==1}">
              		<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex}">${pager.pageIndex}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==2}">
              		<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex-1}">${pager.pageIndex-1}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==3}">
              		<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex-2}">${pager.pageIndex-2}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==4}">
              		<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex-3}">${pager.pageIndex-3}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==0}">
              		<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex-4}">${pager.pageIndex-4}</a></li>
     			  </c:when>
      			 <c:otherwise>
	     		 </c:otherwise>
				</c:choose>
				...
				<c:if test="${pager.pageIndex<pager.totalPage }">
				<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.pageIndex+1}">Next</a></li>
				<li><a href="article!showAllArticleForPageAdmin.action?pageIndex=${pager.totalPage}">最后一页</a></li>
				</c:if>
				</ul>
			</div>
