							...
							if(editID!=""&&editID!=null&&editID!=undefined){
								$.ajax({
									...
									success : function(msg) {
										...
										alert("保存成功!");
										window.location.href="logInfo!showAllLogInfoForPageAdmin.action";
									}
								});
							}
						},
...
<%@include file="/common/header.jsp" %>
    ...
    <div class="container-fluid" id="left">
      <div class="row-fluid">
        <%@include file="/common/houtai_menu.jsp" %>
        <!-- DIY -->
        ...
        <button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"
			...
			<table class="table  table-hover">
				...
				<c:forEach items="${pager.list}" varStatus="logInfos" var="log">
					<tr class="success">
						<td>${log.id}</td>
						<td>${log.userName}</td>
						<td>${log.loginIP}</td>
						...
						<td>
							<button class="btn btn-small btn-primary"
								onclick="toUpdate1('${log.id}','${log.userName}','${log.loginIP}');">
								<span class="ui-button-text">修改</span>
							</button> &nbsp;&nbsp;
							<button class="btn btn-small btn-danger"
								onclick="DelUser1('${log.id}')">
			...
			<div class="pagination" align="center">
				...
				<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=1">首页</a></li>
				<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex-1}">Prev</a></li>
				</c:if>
				<c:if test="${pager.pageIndex>=6}">
				<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex-1}">...</a></li>
				</c:if>
				<c:if test="${pager.pageIndex<=pager.totalPage }">
				<c:choose>
      			 <c:when test="${pager.pageIndex%5==1}">
              		<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex}">${pager.pageIndex}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==2}">
              		<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex-1}">${pager.pageIndex-1}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==3}">
              		<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex-2}">${pager.pageIndex-2}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==4}">
              		<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex-3}">${pager.pageIndex-3}</a></li>
     			  </c:when>
     			  <c:when test="${pager.pageIndex%5==0}">
              		<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex-4}">${pager.pageIndex-4}</a></li>
     			  </c:when>
      			 <c:otherwise>
	     		 </c:otherwise>
				</c:choose>
				...
				<c:if test="${pager.pageIndex<pager.totalPage }">
				<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.pageIndex+1}">Next</a></li>
				<li><a href="logInfo!showAllLogInfoForPageAdmin.action?pageIndex=${pager.totalPage}">最后一页</a></li>
				</c:if>
				</ul>
			</div>
			</div>
