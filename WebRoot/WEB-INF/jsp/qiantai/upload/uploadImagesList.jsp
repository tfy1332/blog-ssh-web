...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>显示上传图片列表Gallery - Beautiful很酷很炫的图片效果----IT is SHow Time</title>
		...
		<!-- /section:settings.box -->
						...
						<h1>
							...
							<div class="row">
								<div class="col-xs-12">
									<table id="sample-table-1" class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												...
												<th>图片ID</th>
												<th>图片HASH</th>
												<th class="hidden-480">图片KEY</th>
												<th class="hidden-480">图片所属用户</th>
												<th class="hidden-480">图片上传IP</th>
												<th class="hidden-480">图片上传时间</th>
												<th>
									...
									<tbody>
										<c:forEach items="${pager.list}" var="image" varStatus="images">
											<tr>
												...
												<td>${image.img_key}</td>
												<td>${image.img_usernmae}</td>
												<td>${image.loginIP}</td>
												<td class="hidden-480">${image.upload_time}</td>
												<td>
												
												<a href="#">
												<img src="http://tfyworld.qiniudn.com/${image.img_key}" class="msg-photo" alt="TFY" />
												</a>
												</td>
			...
			<div class="pagination" align="center">
				<span style="border-style: solid;border-width: 1px 1px 1px 0;float: left;line-height: 20px;padding: 4px 12px;"> ${pager.total}条数据  共${pager.totalPage}页</span>
					...
					<li><a href="upload!showUserImagesForPager.action?pageIndex=1">首页</a></li>
					<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex-1}">Prev</a></li>
					</c:if>
					<c:if test="${pager.pageIndex>=6}">
					<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex-1}">...</a></li>
					</c:if>
					<c:if test="${pager.pageIndex<=pager.totalPage }">
					<c:choose>
	      			 <c:when test="${pager.pageIndex%5==1}">
	              		<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex}">${pager.pageIndex}</a></li>
	     			  </c:when>
	     			  <c:when test="${pager.pageIndex%5==2}">
	              		<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex-1}">${pager.pageIndex-1}</a></li>
	     			  </c:when>
	     			  <c:when test="${pager.pageIndex%5==3}">
	              		<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex-2}">${pager.pageIndex-2}</a></li>
	     			  </c:when>
	     			  <c:when test="${pager.pageIndex%5==4}">
	              		<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex-3}">${pager.pageIndex-3}</a></li>
	     			  </c:when>
	     			  <c:when test="${pager.pageIndex%5==0}">
	              		<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex-4}">${pager.pageIndex-4}</a></li>
	     			  </c:when>
	      			 <c:otherwise>
		     		 </c:otherwise>
					</c:choose>
					...
					<c:if test="${pager.pageIndex<pager.totalPage }">
					<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.pageIndex+1}">Next</a></li>
					<li><a href="upload!showUserImagesForPager.action?pageIndex=${pager.totalPage}">最后一页</a></li>
					</c:if>
					</ul>
			</div>
